...
class CRUDEvent(CRUDBase[models.Event, schemas.EventCreate, schemas.EventUpdate]):
    ...
    # def get_events_for_privileged_role(
    ...
    # ) -> list[models.Event]:
    #     q = db.query(models.Event).filter(models.Event.status == schemas.EventStatus.active)
    #     for f in filters:
    #         q = q.filter(f)
    #     return q.all()
    ...
    # def get_by_user_id(self, db: Session, user_id: int, status: schemas.EventStatus=None):
    #     event_with_feedback = (
    #         db.query(models.Event)
